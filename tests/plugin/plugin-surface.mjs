...
if (!fs.existsSync(path.join(buildDir, "adapter.mjs"))) {
  throw new Error(
    ".test-build/adapter.mjs is missing — run `just test-plugin` (its deps build every artifact this suite loads)",
  );
}
...
// Executable gh stub lives in tests/plugin/gh-stub.cjs; writeGhStub copies
...
function writeGhStub(fixture) {
  ...
  fs.copyFileSync(
    path.join(path.dirname(fileURLToPath(import.meta.url)), "gh-stub.cjs"),
    gh,
  );
...
// snapshots catches any new or removed root entry (a surviving forge temp
// dir, for instance).
...
test("registration", async () => {
  ...
  assert.equal(
    [...tools.values()].every(
      (tool) => typeof tool.parameters?.safeParse === "function",
    ),
    true,
...
test("vcs_write schema", async () => {
  ...
  assert.equal(
    vcsWrite.safeParse({ op: "commit", message: "m", files: ["a"], ref: "x" })
      .success,
    false,
...
test("backend failure surfacing", async () => {
  ...
  assert.match(
    ...
    "vcs_write push without remote surfaces the git failure",
  );
});

test("Api-module partition guards (adapter-reachable only)", async () => {
  const fetchCall = await execTool("vcs_read", { args: ["fetch"] });
  assert.ok(fetchCall.error instanceof Error, "vcs_read fetch is rejected");
  assert.match(
    fetchCall.error.message,
    /vcs_read: fetch updates remote-tracking refs/,
    "vcs_read fetch is rejected names the failure",
  );
  const branchRead = await execTool("vcs_read", { args: ["branch", "x"] });
  assert.ok(
    branchRead.error instanceof Error,
    "vcs_read branch is rejected as mutating",
  );
  assert.match(
    branchRead.error.message,
    /vcs_read: mutating operation rejected/,
    "vcs_read branch is rejected as mutating names the failure",
  );
...
test("forge body lifecycle (success)", async () => {
  const ghDir = fs.mkdtempSync(path.join(os.tmpdir(), "agency-plugin-gh-"));
  const ghLog = path.join(ghDir, "log");
  ...
  try {
    assert.equal(
      prCreate.result.details.exit,
      0,
      "forge pr-create with body succeeds",
    );
...
test("forge body lifecycle (failure)", async () => {
  const failDir = fs.mkdtempSync(path.join(os.tmpdir(), "agency-plugin-gh-"));
  const failLog = path.join(failDir, "log");
  ...
  try {
    assert.ok(
      prFail.error instanceof Error,
      "forge pr-create failure surfaces stderr",
    );
    assert.match(
      prFail.error.message,
      /boom/,
      "forge pr-create failure surfaces stderr names the failure",
    );
    assert.equal(
...
test("workflow tool", async () => {
  const cli = await execTool(
    "workflow",
    { field: "cli" },
    {
      setup: (fixture) => {
        fs.writeFileSync(
          path.join(fixture, ".do-results.json"),
  ...
  assert.ok(
    missingState.error instanceof Error,
    "workflow without state surfaces the actionable error",
  );
  assert.match(
    missingState.error.message,
    /run do-driver init first/,
    "workflow without state surfaces the actionable error names the failure",
  );
});
...
test("agency_driver op concat proof", async () => {
  ...
  assert.ok(
    syncOp.error instanceof Error,
    "agency_driver prepends sync to operands",
  );
  assert.match(
    syncOp.error.message,
    /noVcs is required/,
    "agency_driver prepends sync to operands names the failure",
  );
  const startOp = await execTool("agency_driver", { op: "start", args: [] });
  assert.ok(
    startOp.error instanceof Error,
    "agency_driver prepends start to operands",
  );
  assert.match(
    startOp.error.message,
    /step required/,
    "agency_driver prepends start to operands names the failure",
  );
});

test("vcs_read base (state read through the adapter)", async () => {
  const baseOp = await execTool(
    "vcs_read",
    { args: ["base"] },
    {
      setup: (fixture) => {
