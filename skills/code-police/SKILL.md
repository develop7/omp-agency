...
`--no-elegance` — skip Pass 3 (elegance) entirely. Pass 1 (rules) and Pass 2 (fact-check) still run. Use this when the elegance pass would be redundant because it already ran over this same tree — e.g. a caller that invokes the elegance loop standalone and then `/code-police`. Without it, Pass 3 re-runs the elegance loop on an already-simplified tree, paying a full skill invocation (agent spawn, diff re-read, model tokens) to re-derive a near-guaranteed no-op. When the flag is set, report Pass 3 as `Elegance | – | Skipped (--no-elegance)` in the summary.
...
Spawn Pass 1 and Pass 2 as **two parallel sub-agents** via the `task` tool (`agent: "scout"` — Pass 1 and Pass 2 are read-only audits — no edits). Emit both `task` tool calls in a single response so they run concurrently — sequential spawning halves the parallelism.
...
Pass 3 runs **after** Pass 1 and Pass 2 return. It applies fixes via the inline elegance loop and would race against Pass 1/2's grep-and-read work if run in parallel; sequential is the safer ordering. See "Pass 3: Elegance" below. If `--no-elegance` was passed, skip Pass 3 — only Pass 1 and Pass 2 run.
...
**Run the inline elegance loop.** For each iteration (run 3 iterations):

1. **Understand** — Read through the changed files. Note patterns, repetition, unnecessary complexity, non-idiomatic code.
2. **Research** — Use `web_search` to research what simple, elegant (yet readable!) code looks like for the relevant technology.
...
The "Reviewing principles" (prosecutor stance, no talk-out-of-findings) apply here too.
...
Below the table, reproduce each sub-agent's full findings (the Pass 1 rule table, Pass 2's per-finding list, Pass 3's elegance loop log) verbatim so the orchestrator (`/do`'s police step) can commit each violation as its own fix.
