...
`just nickel-build` self-routes through the pinned Nix toolchain, so it works from a bare
...
differs with host CPU count), so the drift gate
(`just nickel-check`, run by `just drift-check`) compares the
